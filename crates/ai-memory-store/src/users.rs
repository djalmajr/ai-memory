...
mod tests {
    ...
    #[test]
    fn human_auth_v51_migrates_token_only_user() {
        let mut conn = Connection::open_in_memory().unwrap();
        conn.pragma_update(None, "foreign_keys", "ON").unwrap();
    ...
    #[test]
    fn human_auth_v51_empty_store_leaves_bootstrap_incomplete() {
        let mut conn = Connection::open_in_memory().unwrap();
        conn.pragma_update(None, "foreign_keys", "ON").unwrap();
        crate::migrations::run_to(&mut conn, 50).unwrap();
    ...
    #[test]
    fn human_auth_v51_preserves_page_and_audit_author_ids() {
        let mut conn = Connection::open_in_memory().unwrap();
        conn.pragma_update(None, "foreign_keys", "ON").unwrap();
        crate::migrations::run_to(&mut conn, 50).unwrap();
